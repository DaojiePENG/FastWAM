...
if [[ ! -d "$TEXT_EMBEDDING_CACHE" ]]; then
    log "verified T5 embedding cache is missing: $TEXT_EMBEDDING_CACHE"
    exit 2
fi
...
if [[ "$launcher_status" -eq 0 && -s "$REPORT_FILE" ]]; then
    if find "$OUTPUT_DIR/checkpoints" -type f -print -quit 2>/dev/null | grep -q .; then
        log "capacity probe violated the no-checkpoint contract"
        exit 3
    fi
...
payload = {
    ...
    "status": "oom" if oom_detected else "failed",
    "batch_size_per_rank": int(sys.argv[5]),
    "world_size": 8,
    "gradient_accumulation_steps": 1,
    ...
    "fallback": (
        ...
        if oom_detected and int(sys.argv[5]) == 20
        else None
    ),
}
