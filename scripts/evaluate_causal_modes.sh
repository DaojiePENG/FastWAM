...
EVAL_ROOT="${EVAL_ROOT:-$ROOT_DIR/evaluate_results/causal_full_history_step${FINAL_STEP}_trials${NUM_TRIALS}}"
...
while ! training_complete; do
    ...
    log "waiting for controlled causal checkpoints (${progress[*]})"
    sleep "$POLL_SECONDS"
done

log "all controlled causal checkpoints are complete; starting LIBERO-Long evaluation"
...
build_mode_fingerprint() {
    ...
    local expected config_name task_choice
    local -a mode_args
...
run_task() {
    ...
    local checkpoint checkpoint_sha256 config_name task_choice
...
"$ROOT_DIR/.venv/bin/python" "$ROOT_DIR/experiments/leapbot/pareto.py" \
    ...
    --require-profiled
log "causal-mode comparison complete: $EVAL_ROOT/pareto/results.csv"
